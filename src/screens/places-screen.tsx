...
import { PlacePlanSheet } from '@/components/place-plan-sheet';
import { queueItineraryArrival } from '@/utils/itinerary-arrival';
...
export default function PlacesScreen() {
  ...
  const { canEdit, places, items, updatePlace, selectedTrip } = useTravel();
  ...
  const open = (event: GestureResponderEvent, place?: Place) => { setDetailOrigin(captureDetailOrigin(event)); setEditing(place ?? 'new'); };
  const finishPlanning = (tripId: string, itemId: string, inserted: boolean) => {
    const arrival = inserted ? queueItineraryArrival(tripId, itemId) : undefined;
    setPlanning(null);
    if (inserted) toast('しおりに追加しました');
    router.push({ pathname: '/trips/[tripId]/itinerary', params: { tripId, itemId, ...(arrival ? { arrival } : {}) } });
  };
  return <View style={styles.screen}>
    <ScrollView testID="places-scroll" contentContainerStyle={[styles.content, { paddingTop: headerHeight + 24 }]} showsVerticalScrollIndicator={false}>
      ...
      {!places.length ? <View style={styles.empty}><View style={styles.emptyPlaceMark} accessibilityElementsHidden importantForAccessibility="no-hide-descendants"><SymbolView name={{ ios: 'mappin.and.ellipse', android: 'location_on', web: 'location_on' }} size={46} tintColor={palette.ocean} /><View style={styles.emptyPlaceAdd}><SymbolView name={{ ios: 'plus', android: 'add', web: 'add' }} size={18} tintColor={palette.onOcean} /></View></View><Text style={styles.emptyTitle}>気になる場所を保存</Text><Pressable accessibilityRole="button" disabled={!canEdit} onPress={(event) => open(event)} style={styles.primary}><Text style={styles.primaryText}>＋ 場所を追加</Text></Pressable></View> : !filtered.length ? <View style={styles.empty}><Text style={styles.emptyTitle}>該当する場所がありません</Text><Pressable accessibilityRole="button" onPress={() => { setFilter('all'); setSearch(''); }} style={styles.primary}><Text style={styles.primaryText}>絞り込みを解除</Text></Pressable></View> : <View testID="place-grid" style={{ gap: 16 }}>{filtered.map((place) => {
        ...
        return <SurfaceCard key={place.id} testID="place-card">
          <CardContent title={place.title} icon={<SymbolView name={{ ios: 'mappin', android: 'location_on', web: 'location_on' }} size={20} tintColor={palette.smoke} />}
            onPress={(event) => open(event, place)} accessibilityLabel={`${place.title}の詳細を開く`} meta={place.openingHours || undefined}>
            {place.note ? <Text numberOfLines={2} style={styles.note}>{place.note}</Text> : null}
            <Text style={[styles.reservation, place.reservationStatus === 'needed' && styles.needed]}>{reservation.label}</Text>
          </CardContent>
          <View style={styles.cardFooter}>
            <Pressable accessibilityRole="button" accessibilityLabel={`${place.title}のステータスを変更`} disabled={!canEdit} onPress={(event) => { setActionOrigin(captureDetailOrigin(event)); setStatusPlace(place); }} style={[styles.status, place.status === 'visited' && styles.statusVisited]}><PlaceStatusIcon status={place.status} /><Text style={styles.statusText}>{status.label}</Text></Pressable>
            <View style={styles.cardActions}>{itineraryItem || canEdit ? <Pressable accessibilityRole="button" onPress={(event) => {
    ...
    {canEdit ? <FloatingAddButton label="場所を追加" onPress={(event) => open(event)} /> : null}
    <MotionPresence>{editing ? <PlaceSheet detailOrigin={detailOrigin} key={editing === 'new' ? 'new' : editing.id} place={editing === 'new' ? undefined : places.find((place) => place.id === editing.id) ?? editing} onClose={() => setEditing(null)} onPlan={canEdit || (editing !== 'new' && items.some((item) => item.id === editing.itineraryItemId)) ? (place, origin) => {
...
const createStyles = (palette: Palette) => StyleSheet.create({
  ...
  filters: { gap: 6 },
  filter: { flexDirection: 'row', alignItems: 'center', gap: 6, borderRadius: 12, paddingHorizontal: 14, paddingVertical: 12, backgroundColor: palette.mist },
  ...
  serial: { color: palette.smoke, fontSize: 12 },
  reservation: { color: palette.smoke, fontSize: 12 },
  needed: { color: palette.warning },
  placeTitle: { color: palette.ink, fontSize: 20, lineHeight: 28, fontWeight: '600' },
  ...
  cardFooter: { padding: 8, paddingHorizontal: 12, borderTopWidth: StyleSheet.hairlineWidth, borderColor: palette.ash, flexDirection: 'row', flexWrap: 'wrap', gap: 8, alignItems: 'center', justifyContent: 'space-between' },
  status: { flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: 12, minHeight: 44, justifyContent: 'center', backgroundColor: palette.soft, borderRadius: 10 },
  statusVisited: { backgroundColor: palette.mist },
  statusText: { color: palette.ink, fontSize: 12, fontWeight: '600' },
  cardActions: { flexDirection: 'row', gap: 8 },
  action: { flexDirection: 'row', alignItems: 'center', gap: 5, paddingHorizontal: 6, minHeight: 44, justifyContent: 'center' },
  actionText: { color: palette.ocean, fontSize: 12, fontWeight: '600' },
  empty: { paddingVertical: 60, alignItems: 'center', gap: 24 },
  emptyPlaceMark: { width: 96, height: 96, borderRadius: 48, backgroundColor: palette.mist, alignItems: 'center', justifyContent: 'center' },
