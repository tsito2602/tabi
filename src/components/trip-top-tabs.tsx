  ...
  return <View testID="trip-header" style={[styles.shell, { paddingTop: insets.top }]}>
    <View testID="trip-header-inner" style={styles.inner}>
      <View style={styles.topRow}>
        <Pressable accessibilityRole="button" accessibilityLabel={managing ? 'しおりへ戻る' : '旅行一覧へ戻る'} onPress={() => managing ? router.replace({ pathname: '/trips/[tripId]/itinerary', params: { tripId } }) : navigateTrip(tripId, 'close', () => router.replace('/'))} testID="trip-back" style={styles.backButton}><Text style={styles.backMark}>‹</Text></Pressable>
        <View testID="trip-heading" style={styles.title}><Text testID={managing ? undefined : "trip-title"} numberOfLines={1} style={styles.tripName}>{managing ? 'メンバー' : selectedTrip?.name}</Text><Text style={styles.tripDates}>{managing ? selectedTrip?.name : `${selectedTrip?.startsOn.replaceAll('-', '.')} — ${selectedTrip?.endsOn.replaceAll('-', '.')}`}</Text></View>
