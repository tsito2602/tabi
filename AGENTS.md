...
- Web/PWAの自動検証・配信はWorkers Buildsを標準とする。細かな調整は作業ブランチでまとめ、stagingへは確認した一区切りだけを反映する。非本番ブランチの自動Build、定期実行、検証用の一時Actions workflow、重複デプロイを追加しない。手動Actionsは明示依頼時のみ。
